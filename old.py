...
def get_gemini_response(prompt, history, tHistory, m="gemini-2.0-flash"):
    if tHistory != "na":
        query = f"""
{AIprompt}
...
def process_and_reply(channel_id, text, ts, thread_ts):
    ...
    client.chat_postMessage(
        channel=channel_id,
        text=get_gemini_response(text, history, tHistory, m="gemini-2.0-flash"),
        thread_ts=ts,
    )
...
def msg(payload):
    ...
    search_result = client.search_messages(
        token=os.getenv("USER_OAUTH_TOKEN"), query=searchQuery, count=10
